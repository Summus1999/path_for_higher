...
public:
    vector<vector<int>> fourSum(vector<int>& nums, int target) {
        ...
        for (int a = 0; a <= n - 4; a++) {
            ...
            // 剩余数字取最小仍大于target，直接结束
            if (x + nums[a + 1] + nums[a + 2] + nums[a + 3] > target) {
                break;
            }
            // 剩余数字取最大仍小于target，继续下一个
...
public:
    int totalFruit(vector<int>& fruits) {
        ...
        // right 作为窗口右边界，遍历所有果树
        for (int right = 0; right < n; right++) {
            hash[fruits[right]]++;
            while (hash.size() > 2) {
                auto out = fruits[left];
                hash[out]--;
                // 若该水果在窗口中已无残留，彻底移除
                if (hash[out] == 0) {
                    hash.erase(out);
...
public:
    int diameterOfBinaryTree(TreeNode* root) {
        ...
        auto dfs = [&](this auto&& dfs, TreeNode* node) -> int {
            if (node == nullptr) {
                return -1;  // 空节点返回-1，使叶子节点的边数为0
            }
            // 计算从当前节点出发，向左/右的最长路径边数
            int leftLength = dfs(node->left) + 1;
            int rightLength = dfs(node->right) + 1;
            ans = max(ans, rightLength + leftLength);  // 更新直径
            return max(leftLength, rightLength);  // 返回单边最长
        };
        dfs(root);
        return ans;
    }
...
public:
    int minCameraCover(TreeNode* root) {
        auto dfs = [&](this auto&& dfs,
            ...
            // 递归获取左右子树的三种状态
            auto [left_place, left_covered_by_parent,
                  left_covered_by_children] = dfs(node->left);
            auto [right_place, right_covered_by_parent,
                  right_covered_by_children] = dfs(node->right);
            int place_camera = min(left_place, left_covered_by_parent) +
                               min(right_place, right_covered_by_parent) + 1;
            int covered_by_parent = min(left_place, left_covered_by_children) +
                                    min(right_place, right_covered_by_children);
            int covered_by_children =
                min({left_place + right_covered_by_children,
                     left_covered_by_children + right_place,
                     left_place + right_place});
            return {place_camera, covered_by_parent, covered_by_children};
        };
        auto [root_place, root_covered_by_parent, root_covered_by_children] =
            dfs(root);
        return min(root_place, root_covered_by_children);
    }
};
```
...
public:
    int maxProfit(vector<int>& prices) {
        ...
        for (auto cur : prices) {
            // 计算今日卖出后的现金并更新持有状态
            auto newCash = max(ans, cur + curPro);
            curPro = max(curPro, ans - cur);
            ans = newCash;
        }
        return ans;
    }
...
public:
    int rob(TreeNode* root) {
        ...
        auto dfs = [&](this auto&& dfs, TreeNode* node) -> pair<int, int> {
            ...
            // 偷当前节点：子节点不能偷
            int rob = left_not_rob + right_not_rob + node->val;
            // 不偷当前节点：子节点可偷可不偷，取各自的最大值
            int not_rob = max(left_rob, left_not_rob) + max(right_rob, right_not_rob);
            return {rob, not_rob};
        };
        auto [rob_val, not_rob_val] = dfs(root);
...
public:
    int coinChange(vector<int>& coins, int amount) {
        ...
        // 外层遍历硬币，内层遍历金额（完全背包）
        for (auto coin : coins) {
            for (auto cur = coin; cur <= amount; cur++) {
                dp[cur] = min(dp[cur], dp[cur - coin] + 1);
            }
        }
        int minCoin = dp[amount];
        return minCoin < INT_MAX / 2 ? minCoin : -1;
    }
};
```
...
public:
    int minDistance(string word1, string word2) {
        int n = word2.size();
        vector<int> dp(n + 1, 0);
        // 初始化：word1为空时，需要i次插入操作
        for (int i = 0; i < n; i++) {
            dp[i + 1] = i + 1;
        }
        for (auto cur : word1) {
            int pre = dp[0];
            // 更新当前行的 dp[0]：word2为空时需要多一次删除
            dp[0]++;
            for (int i = 0; i < n; i++) {
                int temp = dp[i + 1];
                if (cur == word2[i]) {
                    dp[i + 1] = pre;  // 字符相同，无需操作
                } else {
                    // 删除、插入、替换取最小 + 1
                    dp[i + 1] = min({dp[i + 1], dp[i], pre}) + 1;
                }
                pre = temp;
            }
        }
...
public:
    int maxProfit(vector<int>& prices) {
        ...
        for (auto cur : prices) {
            int newCash = max(ans, curProfit + cur);
            curProfit = max(curProfit, pre - cur);
            // 更新前天未冻结的现金
            pre = ans;
            ans = newCash;
        }
    ...
    int longestPalindromeSubseq(string s) {
        ...
        for (int i = n - 1; i >= 0; i--) {
            ...
            for (int j = i + 1; j < n; j++) {
                int tmp = dp[j];
                if (s[i] == s[j]) {
                    dp[j] = pre + 2;  // 字符匹配：左下角值 + 2
                } else {
                    dp[j] = max(dp[j], dp[j - 1]);  // 不匹配：取左边或下边最大值
                }
                pre = tmp;
            }
        }
    ...
    int minScoreTriangulation(vector<int>& values) {
        ...
        for (int i = n - 3; i >= 0; i--) {
            for (int j = i + 2; j < n; j++) {
                ...
                // 枚举顶点 k 作为三角形的第三个顶点
                for (int k = i + 1; k < j; k++) {
                    int score = f[i][k] + f[k][j] + values[i] * values[k] * values[j];
                    if (score < f[i][j]) {
                        f[i][j] = score;
                    }
                }
...
public:
    vector<string> generateParenthesis(int n) {
        ...
        auto dfs = [&](this auto&& dfs, int i, int open) -> void {
            if (i == m) {
                ans.emplace_back(path); // 直接构造避免拷贝
                return;
            }
            // 添加左括号
            if (open < n) {
...
public:
    vector<vector<int>> combinationSum3(int k, int n) {
        ...
        vector<int> path;
        // 从小到大选择
        auto dfs = [&](this auto&& dfs, int start, int t) -> void {
...
public:
    vector<vector<int>> colorBorder(vector<vector<int>>& grid, int row, int col,
        ...
        auto dfs = [&](this auto&& dfs, int i, int j) -> void {
            if (i < 0 || i >= m || j < 0 || j >= n ||
                grid[i][j] != originColor || visited[i][j] == true) {
                return;
            }
            visited[i][j] = true;
            // 检查是否是边界
            if (i == 0 || i == m - 1 || j == 0 || j == n - 1) {
                ...
                if (grid[i + 1][j] != originColor ||
                    grid[i - 1][j] != originColor ||
                    grid[i][j + 1] != originColor ||
                    grid[i][j - 1] != originColor) {
                    point[i][j] = true;
                }
            }
...
public:
    int fourSumCount(vector<int>& nums1, vector<int>& nums2, vector<int>& nums3,
                     vector<int>& nums4) {
        // 记录 nums1 和 nums2 中所有两数之和的出现次数
        unordered_map<int, int> hash;
        int ans = 0;
        // 遍历 nums1 和 nums2，计算所有可能的和并统计
        for (int x : nums1) {
            for (int y : nums2) {
                hash[x + y]++;
            }
        }
        // 遍历 nums3 和 nums4，计算需要的补数（-x-y）并累加结果
        for (int x : nums3) {
            for (int y : nums4) {
                ans += hash[-x - y];
            }
